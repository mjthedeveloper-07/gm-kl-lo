...
for (let i = MIN_HISTORY; i < chrono.length; i++) {
  ...
  if ((i - MIN_HISTORY + 1) % 500 === 0) console.log(`  ...${i - MIN_HISTORY + 1} draws scored`);
}
...
const L: string[] = [];
L.push("=".repeat(72));
L.push(`L4 DAILY HIT PLAYBOOK — ${ranked.length}-METHOD LEADERBOARD`);
...
const mapping = `L4 COMPLEX-NUMBER FORMULA MAPPING (Methods 17-22)
============================================================
Each method treats an L4 tail "abcd" as the complex number
z = ab + cd·i, applies one identity from the matematica.pt sheet,
and prepends the most common pos-0/pos-1 prefix from the last
200 draws to form a 6-digit candidate.

Method 17 — L4 Complex Polar Drift
Identity:    z = |z|·(cos θ + i sin θ)
Tracks (|z|, θ) of the last 50 L4 tails, projects forward in
polar space, converts back to L4 tails.

Method 18 — L4 Conjugate Mirror
Identity:    z̄ = a − bi    and    −z = (−a) + (−b)i
Picks the 5 hottest recency-weighted L4 tails and emits their
mod-100 conjugate and negative reflections.

Method 19 — L4 nth-Roots Generator
Identity:    ⁿ√z = ⁿ√|z|·e^(i(θ + 2kπ)/n),  k = 0..n−1
Treats the most recent L4 tail as z, emits its 5 fifth-roots.

Method 20 — L4 z₁·z₂ Angular Drift
Identity:    z₁·z₂ = |z₁|·|z₂|·e^i(θ₁ + θ₂)
Multiplies the last two L4 tails and fans out 5 candidates by
rotating around the product angle.

Method 21 — L4 Inverse Reflection  (NEW)
Identity:    z⁻¹ = z̄ / |z|²
Computes the inverse direction of each top-5 hot L4 tail and
emits as a candidate.

Method 22 — L4 Quotient Drift  (NEW)
Identity:    z₁ / z₂ = (|z₁|/|z₂|) · e^i(θ₁ − θ₂)
Divides the last two L4 tails to extract polar drift, then
projects 5 candidates forward.
`;
fs.writeFileSync("/mnt/documents/l4-formula-mapping.txt", mapping);

console.log("\n=== TOP 7 ===");
ranked.slice(0, 7).forEach((r, i) =>
  console.log(`  ${i+1}. ${r.method.padEnd(35)} composite ${r.composite.toFixed(2)}  hits ${r.hits}/${r.evaluated}  lift ${r.lift.toFixed(2)}×  recent ${r.recentLift.toFixed(2)}×`));
